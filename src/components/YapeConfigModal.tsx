  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl max-w-lg w-full p-6 md:p-8 shadow-2xl border border-gray-200 space-y-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-serif font-black text-lg text-[#2D473C]">
                {isCashier ? "Alternar QR de Yape" : "Configurar QR de Yape"}
              </h3>
              <p className="text-xs text-gray-500">
                {isCashier
                  ? "Selecciona el QR activo configurado para los pagos de clientes"
                  : "Modo Administrador: Alternar, editar datos y cargar imagen de QR"}
              </p>
            </div>
          </div>
          <button
        ...
        {/* Selector de Modo (Empresa vs Personal) */}
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            ...
            {config.businessQrUrl && (
              <div className="absolute right-2 bottom-2 w-7 h-7 opacity-20 pointer-events-none">
                <img src={config.businessQrUrl} alt="Thumb" className="w-full h-full object-contain" />
              </div>
            )}
          </button>
          ...
          >
            ...
            <div>
              ...
              {config.personalPhone && (
                <span className="text-[11px] text-purple-700 font-semibold block mt-0.5">
                  Tel: {config.personalPhone}
                </span>
              )}
            </div>
            {config.personalQrUrl && (
              <div className="absolute right-2 bottom-2 w-7 h-7 opacity-20 pointer-events-none">
                <img src={config.personalQrUrl} alt="Thumb" className="w-full h-full object-contain" />
              </div>
            )}
          </button>
        </div>

        {/* VISTA EN MODO CAJA (SOLO LECTURA Y CONMUTACIÓN RÁPIDA) */}
        {isCashier ? (
          <div className="p-4 rounded-2xl bg-[#F9F8F3] border border-[#EBE7D8] space-y-4">
            <span className="text-xs font-bold text-gray-600 uppercase tracking-wider block">
              QR Seleccionado ({isPersonal ? "Personal" : "Empresa"})
            </span>

            <div className="flex items-center gap-4 bg-white p-3.5 rounded-2xl border border-gray-200">
              ...
              <div className="flex-1 space-y-1 text-xs">
                ...
                <p className="font-bold text-sm text-gray-900">
                  {isPersonal ? config.personalName : config.businessName}
                </p>
                {(isPersonal ? config.personalPhone : config.businessPhone) && (
                ...
                <div className="pt-1">
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-extrabold uppercase tracking-wider ${
                      isPersonal ? "bg-purple-100 text-purple-800" : "bg-emerald-100 text-emerald-800"
                    }`}
                  >
                    Activo al confirmar
                  </span>
            ...
            <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-amber-900 text-xs leading-relaxed flex items-start gap-2">
              ...
              <p>
                Al confirmar, el carrito de compras de los clientes mostrará de inmediato este QR y sus datos al momento de pagar por Yape.
              </p>
            </div>
          </div>
        ) : (
          /* VISTA EN MODO ADMINISTRADOR (EDICIÓN Y CARGA DE IMÁGENES) */
          <>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          >
            ...
            <span>{isCashier ? "Confirmar y Activar QR" : "Aplicar y Guardar Configuración"}</span>
          </button>
        </div>
